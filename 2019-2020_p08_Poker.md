...
2.2 Añada métodos a `poker-hand.js` llamados `hasPair()`, `hasTwopair()`, etc. que devuelven Verdadero o Falso según si la mano cumple o no los criterios pertinentes. 
...
Imprima una tabla de las clasificaciones y sus probabilidades y muestre esos resultados a través de un fichero JSON.
...
Compare sus resultados con los valores que se indican [en Wikipedia](http://en.wikipedia.org/wiki/Hand_rankings).
